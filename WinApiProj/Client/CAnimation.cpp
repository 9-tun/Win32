...
void CAnimation::Load(const wstring & _strRelativePath)
{
	...
	m_pTex = CResMgr::GetInst()->LoadTexture(strTexKey, strTexPath);	// LoadTexture는 있으면 찾아서 반환, 없으면 Loading해서 반환
	
	// 프레임 개수
	FScanf(szBuff, pFile);
	int iFrameCount = 0;
	fscanf_s(pFile, "%d", &iFrameCount);	//숫자만 읽어오기
